...
<div class="container">
    ...
    <div class="row">
        <div class="offset-10 col-12">
            <a href="{{ route('scheduling.createAndAlterView') }}">
                <img class="img-fluid"
    ...
    <div class="container my-3">
        <div class="col-12">
            ...
            <table class="table table-striped table-bordered">
                ...
                <tbody>
                    {{-- {{dd($schedulings)}} --}}
                    @if(isset($schedulings))
                        @foreach ($schedulings as $scheduling)
                            <tr>
                                <td>
                                    {{$scheduling->patient_name}}
                                </td>
                                <td>
                                    {{$scheduling->doctor_name}}
                                </td>
                                <td>
                                    {{$scheduling->date_of_scheduling}}
                                </td>
                                <td>
                                    <form action="{{ route('scheduling.createAndAlterView', $scheduling->id) }}"
                                          id="editForm{{$scheduling->id}}"
                                          method="GET">
                                      @CSRF
                                      <a type="submit" role="button" onclick="$('#editForm{{$scheduling->id }}').submit()">
                                          <img class="img-fluid imgEdit"
                                ...
                                <td>
                                    <form action="{{ route('scheduling.delete', $scheduling->id) }}"
                                          id="deleteForm{{$scheduling->id}}"
                                          method="POST">
                                        @CSRF
                                        @METHOD('DELETE')
                                        <a type="submit" role="button" onclick="$('#deleteForm{{$scheduling->id }}').submit()">
                                            <img class="img-fluid imgDelete"
